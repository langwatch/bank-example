...
async def test_fraud_investigation_workflow():
    ...
    def check_customer_exploration_called(state: scenario.ScenarioState):
        ...
        assert state.has_tool_call("explore_customer_account"), "Agent should call explore_customer_account for fraud concerns"
        ...
        if tool_call:
            ...
            assert "fraud" in args.get("query", "").lower() or "security" in args.get("query", "").lower(), \
    ...
    def verify_no_inappropriate_tools(state: scenario.ScenarioState):
        ...
        assert not state.has_tool_call("get_message_suggestion"), \
    ...
    result = await scenario.run(
        ...
        script=[
            scenario.user("Hi, I just checked my account and there are transactions I didn't make. I think my card was stolen!"),
            scenario.agent(),
            check_customer_exploration_called,
            scenario.user("There's an $85 charge at Amazon and a $45 charge at some gas station. I definitely didn't make these purchases."),
            scenario.agent(),
            scenario.user("Yes, please help me secure my account right away. I'm worried about more charges."),
            scenario.agent(),
...
async def test_complex_issue_triggers_knowledge_base():
    def check_message_suggestion_called(state: scenario.ScenarioState):
        ...
        assert state.has_tool_call("get_message_suggestion"), \
        ...
        if tool_call:
            ...
            assert any(keyword in query for keyword in ["lock", "fee", "deposit", "multiple"]), \
    ...
    result = await scenario.run(
        ...
        script=[
            scenario.user("I have multiple problems with my account. My online banking is locked, there's a $35 fee I don't understand, and my paycheck didn't deposit."),
            scenario.agent(),
            check_message_suggestion_called,
            scenario.user("I've tried resetting my password multiple times and I really need access to pay my bills. This is really stressing me out."),
            scenario.agent(),
            scenario.judge(),
        ],
    )

    assert result.success, f"Complex issue test failed: {result.failure_reason}"
...
async def test_escalation_workflow():
    def check_escalation_called(state: scenario.ScenarioState):
        ...
        assert state.has_tool_call("escalate_to_human"), \
        ...
        if tool_call:
            ...
            assert any(keyword in reason for keyword in ["frustrated", "manager", "human", "escalation"]), \
    ...
    result = await scenario.run(
        ...
        script=[
            scenario.user("I've been calling about this same issue for two weeks and nobody can fix it. I want to speak to a real person who can actually help me!"),
            scenario.agent(),
            scenario.user("No more troubleshooting! I want a manager or supervisor right now. This is unacceptable service."),
            scenario.agent(),
...
async def test_comprehensive_tool_coordination():
    ...
    def validate_tool_coordination(state: scenario.ScenarioState):
        ...
        assert "explore_customer_account" in tools_called, \
    ...
    result = await scenario.run(
        ...
        script=[
            scenario.user("I want to get better at managing my money. Can you analyze my spending and help me understand where I can improve?"),
            scenario.agent(),
            track_customer_exploration,
            scenario.user("That's helpful! Can you also suggest a realistic budget based on my spending patterns and give me specific advice?"),
            scenario.agent(),
            track_message_suggestion,
            scenario.user("This conversation has been really valuable. Can you summarize the key insights and recommendations we discussed?"),
            scenario.agent(),
...
async def test_urgent_business_scenario():
    def check_appropriate_urgency_response(state: scenario.ScenarioState):
        ...
        assert has_escalation or has_exploration, \
        ...
        if has_escalation:
            ...
            if tool_call:
                ...
                assert urgency == "high", "Business urgency should be marked as high priority"
    ...
    result = await scenario.run(
        ...
        script=[
            scenario.user("URGENT: My business account is frozen and I need to pay my employees today. This is costing me money every minute!"),
            scenario.agent(),
            check_appropriate_urgency_response,
            scenario.user("I can't wait. My payroll is due in 2 hours and my employees are depending on me. What can you do right now?"),
            scenario.agent(),
            scenario.judge(),
        ],
    )
...
async def test_tool_precision_simple_query():
    ...
    def verify_minimal_tool_usage(state: scenario.ScenarioState):
        ...
        assert tool_calls <= 1, f"Agent should use minimal tools for simple queries, but used {tool_calls} tool calls"
